function[points,sequence]=mactropolis(inputx,inputy)
%CKH 10-13-2012
...
for i=1:N  %go over the entire collection of macaroni segments
  ...
  else PositiveOrientation=false; 
  end    
  
  %set an energy for the Metropolis algorithm based on total squared
  %error of ArcN points, each a distance seglength from the curve
  kt=ArcN*Seglength^2; 
  kt=0.0027*kt%then scale it to set a temperature
  
  %Later I can lower the temperature...
   
  probFlip=exp(-1*abs(TotalPosError-TotalNegError)/kt);
  if (rand<probFlip)
